...
private:
	struct listItem {
		...
		{ this->data = data; this->nextItem = nextItem; }
	};
...
public:
	...
	bool AddAfterPosition(const unsigned int index, const int data);		//Додати після позиції

	//Функції видалення
	bool DeleteFromStart();										//Видалити з початку
	bool DeletePosition(const unsigned int index);			//Видалити позицію
